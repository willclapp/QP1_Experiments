...
const writeResults = (word, coronal, labial, dorsal, assets) => {
	fs.writeFile('B_left_G_first/js/stims.js', buildOutput(word, coronal, labial, dorsal, assets), err => {
		if (err) {
			console.error(err)
		}
...
fs.createReadStream('B_left_G_first/trial_csv/exposure_B_left.csv')
	.pipe(csv())
	.on('data', data => {
	...
	.on('end', () => {
		console.log('done reading exposure_B_left.csv')
		fs.createReadStream('B_left_G_first/trial_csv/test_B_left_coronal.csv')
			.pipe(csv())
			.on('data', data => {
			...
			.on('end', () => {
				console.log('done reading test_B_left_coronal.csv')
				fs.createReadStream('B_left_G_first/trial_csv/test_B_left_labial.csv')
					.pipe(csv())
					.on('data', data => {
					...
					.on('end', () => {
						console.log('done reading test_B_left_labial.csv')
						fs.createReadStream('B_left_G_first/trial_csv/test_B_left_dorsal.csv')
							.pipe(csv())
							.on('data', data => {
							...
							.on('end', () => {
								console.log('done reading test_B_left_dorsal.csv')
