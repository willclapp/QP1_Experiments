...
const writeResults = (word, coronal, labial, dorsal, assets) => {
	fs.writeFile('control_group_left_break/js/stims.js', buildOutput(word, coronal, labial, dorsal, assets), err => {
		if (err) {
			console.error(err)
		}
...
fs.createReadStream('control_group_left_break/trial_csv/exposure_control_left.csv')
	.pipe(csv())
	.on('data', data => {
		exposureResults.push(data)
		// This is just for preloading!
		assetPaths.push([
			'audio/exposure_control_group/' + data.path, 
		])
	})
	.on('end', () => {
		console.log('done reading exposure_control_left.csv')
		fs.createReadStream('control_group_left_break/trial_csv/test_control_group_left_coronal.csv')
			.pipe(csv())
			.on('data', data => {
			...
			.on('end', () => {
				console.log('done reading test_control_group_left_coronal.csv')
				fs.createReadStream('control_group_left_break/trial_csv/test_control_group_left_labial.csv')
					.pipe(csv())
					.on('data', data => {
					...
					.on('end', () => {
						console.log('done reading test_control_group_left_labial.csv')
						fs.createReadStream('control_group_left_break/trial_csv/test_control_group_left_dorsal.csv')
							.pipe(csv())
							.on('data', data => {
							...
							.on('end', () => {
								console.log('done reading test_control_group_left_dorsal.csv')
