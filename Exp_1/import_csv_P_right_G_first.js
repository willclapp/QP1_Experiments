...
const writeResults = (word, coronal, labial, dorsal, assets) => {
	fs.writeFile('P_right_G_first/js/stims.js', buildOutput(word, coronal, labial, dorsal, assets), err => {
		if (err) {
			console.error(err)
		}
...
fs.createReadStream('P_right_G_first/trial_csv/exposure_P_right.csv')
	.pipe(csv())
	.on('data', data => {
	...
	.on('end', () => {
		...
		fs.createReadStream('P_right_G_first/trial_csv/test_P_right_coronal.csv')
			.pipe(csv())
			.on('data', data => {
			...
			.on('end', () => {
				...
				fs.createReadStream('P_right_G_first/trial_csv/test_P_right_labial.csv')
					.pipe(csv())
					.on('data', data => {
					...
					.on('end', () => {
						...
						fs.createReadStream('P_right_G_first/trial_csv/test_P_right_dorsal.csv')
							.pipe(csv())
							.on('data', data => {
