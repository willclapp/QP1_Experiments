...
const writeResults = (word, coronal, labial, dorsal, assets) => {
	fs.writeFile('P_left_D_first/js/stims.js', buildOutput(word, coronal, labial, dorsal, assets), err => {
		if (err) {
			console.error(err)
		}
...
fs.createReadStream('P_left_D_first/trial_csv/exposure_P_left.csv')
	.pipe(csv())
	.on('data', data => {
	...
	.on('end', () => {
		...
		fs.createReadStream('P_left_D_first/trial_csv/test_P_left_coronal.csv')
			.pipe(csv())
			.on('data', data => {
			...
			.on('end', () => {
				...
				fs.createReadStream('P_left_D_first/trial_csv/test_P_left_labial.csv')
					.pipe(csv())
					.on('data', data => {
					...
					.on('end', () => {
						...
						fs.createReadStream('P_left_D_first/trial_csv/test_P_left_dorsal.csv')
							.pipe(csv())
							.on('data', data => {
