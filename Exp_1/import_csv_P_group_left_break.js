...
const writeResults = (word, coronal, labial, dorsal, assets) => {
	fs.writeFile('P_group_left_break/js/stims.js', buildOutput(word, coronal, labial, dorsal, assets), err => {
		if (err) {
			console.error(err)
		}
...
fs.createReadStream('P_group_left_break/trial_csv/exposure_P_group_left.csv')
	.pipe(csv())
	.on('data', data => {
	...
	.on('end', () => {
		...
		fs.createReadStream('P_group_left_break/trial_csv/test_P_group_left_coronal.csv')
			.pipe(csv())
			.on('data', data => {
			...
			.on('end', () => {
				console.log('done reading test_P_group_left_coronal.csv')
				fs.createReadStream('P_group_left_break/trial_csv/test_P_group_left_labial.csv')
					.pipe(csv())
					.on('data', data => {
					...
					.on('end', () => {
						console.log('done reading test_P_group_left_labial.csv')
						fs.createReadStream('P_group_left_break/trial_csv/test_P_group_left_dorsal.csv')
							.pipe(csv())
							.on('data', data => {
							...
							.on('end', () => {
								console.log('done reading test_P_group_left_dorsal.csv')
