...
const writeResults = (word, nonword, assets) => {
	fs.writeFile('B_left/js/stims.js', buildOutput(word, nonword, assets), err => {
		if (err) {
			console.error(err)
		}
...
fs.createReadStream('B_left/trial_csv/exposure_B_left.csv')
	.pipe(csv())
	.on('data', data => {
	...
	.on('end', () => {
		...
		fs.createReadStream('B_left/trial_csv/test_B_left.csv')
			.pipe(csv())
			.on('data', data => {
			...
			.on('end', () => {
				console.log('done reading test_B_left.csv')
