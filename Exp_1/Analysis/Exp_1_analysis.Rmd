...
```{r}
df.data <- read.csv("/Users/willclapp/Desktop/QP1/QP1_Experiments/Exp_1/Analysis/Exp_1_preprocessed.csv")
...
p2 = ggplot(data = df.PB,
  ...
  geom_smooth(aes(color = group),
              ...
              method.args = list(family = "binomial")) +
  geom_point(alpha = 0.02) +
  labs(title = "[pV]-[bV]", x = "Normalized Step", y = "Likelihood \"B\" response") +
  theme_bw() + 
  scale_color_manual(values = c("B_group" = "#eb7a7a", "control" = "#7ac9eb", "P_group" = "#e6bd43"))
...
p3 = ggplot(data = df.TD,
  ...
  geom_smooth(aes(color = group),
              ...
              method.args = list(family = "binomial")) +
  geom_point(alpha = 0.02) +
  labs(title = "[tV]-[dV]", x = "Normalized Step", y = "Likelihood \"D\" response") +
  theme_bw() + 
  theme(legend.position = "none") +
  scale_color_manual(values = c("B_group" = "#eb7a7a", "control" = "#7ac9eb", "P_group" = "#e6bd43"))
...
p4 = ggplot(data = df.KG,
  ...
  geom_smooth(aes(color = group),
              ...
              method.args = list(family = "binomial")) +
  geom_point(alpha = 0.02) +
  labs(title = "[kV]-[gV]", x = "Normalized Step", y = "Likelihood \"G\" response") +
  theme_bw() + 
  theme(legend.position = "none") +
  scale_color_manual(values = c("B_group" = "#eb7a7a", "control" = "#7ac9eb", "P_group" = "#e6bd43"))
...
# glmer.listeners_int = glmer(response_01 ~ 1 + group * norm_step + (1|participant_id:norm_step),
#                      data = df.data,
#                      family="binomial")
# glmer.PB_int = glmer(response_01 ~ 1 + group * norm_step + (1|participant_id:norm_step),
#                      data = df.PB,
#                      family="binomial")
# glmer.TD_int = glmer(response_01 ~ 1 + group * norm_step + (1|participant_id:norm_step),
#                      data = df.TD,
#                      family="binomial")
# glmer.KG_int = glmer(response_01 ~ 1 + group * norm_step + (1|participant_id:norm_step),
#                      data = df.KG,
#                      family="binomial")
# 
# summary(glmer.listeners_int)
# summary(glmer.PB_int)
# summary(glmer.TD_int)
# summary(glmer.KG_int)
...
```{r}
df.middle <- df.data %>% 
  filter(norm_step > -0.5 & norm_step < 0.5)
...
Visualize using averages instead of glm
