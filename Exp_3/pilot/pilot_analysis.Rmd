...
```{r}
df.data <- read.csv("/Users/willclapp/Desktop/QP1/QP1_Experiments/Exp_3/pilot/Exp_3_pilot_preprocessed.csv")
...
```{r}
df.onset = df.data %>% 
  filter(test_part == "test_onset")
df.coda = df.data %>% 
  filter(test_part == "test_coda")
...
ggplot(data = df.onset,
  ...
  theme_bw() 
  # scale_color_manual(values = c("B_group" = "#eb7a7a", "control" = "#7ac9eb"))
...
glm.norm_listeners = glm(response_01 ~ 1 + group * norm_step, 
                     data = df.data,
                     family="binomial")
glm.norm_onset = glm(response_01 ~ 1 + group * norm_step, 
                     data = df.onset,
                     family="binomial")
glm.norm_coda = glm(response_01 ~ 1 + group * norm_step, 
                     data = df.coda,
                     family="binomial")

summary(glm.norm_listeners)
summary(glm.norm_onset)
summary(glm.norm_coda)
...
What if we just do first blocks?

```{r}
df.onset_first <- df.onset %>% 
  filter(block_order=="onset-coda")

df.coda_first <- df.coda %>% 
  filter(block_order=="coda-onset")
...
glmer.listeners = glmer(response_01 ~ 1 + group * norm_step + (1 | participant_id), 
                     data = df.data,
                     family="binomial")
glmer.onset = glmer(response_01 ~ 1 + group * norm_step + (1 | workerid), 
                     data = df.onset,
                     family="binomial")
glmer.coda = glmer(response_01 ~ 1 + group * norm_step + (1 | workerid), 
                     data = df.coda,
                     family="binomial")
