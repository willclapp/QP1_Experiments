...
const writeResults = (word, coronal, labial, dorsal, assets) => {
	fs.writeFile('control_right_D_first/js/stims.js', buildOutput(word, coronal, labial, dorsal, assets), err => {
		if (err) {
			console.error(err)
		}
...
fs.createReadStream('control_right_D_first/trial_csv/exposure_control_right.csv')
	.pipe(csv())
	.on('data', data => {
	...
	.on('end', () => {
		...
		fs.createReadStream('control_right_D_first/trial_csv/test_control_right_coronal.csv')
			.pipe(csv())
			.on('data', data => {
			...
			.on('end', () => {
				...
				fs.createReadStream('control_right_D_first/trial_csv/test_control_right_labial.csv')
					.pipe(csv())
					.on('data', data => {
					...
					.on('end', () => {
						...
						fs.createReadStream('control_right_D_first/trial_csv/test_control_right_dorsal.csv')
							.pipe(csv())
							.on('data', data => {
