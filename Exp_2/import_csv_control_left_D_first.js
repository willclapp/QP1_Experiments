...
const writeResults = (word, coronal, labial, dorsal, assets) => {
	fs.writeFile('control_left_D_first/js/stims.js', buildOutput(word, coronal, labial, dorsal, assets), err => {
		if (err) {
			console.error(err)
		}
...
fs.createReadStream('control_left_D_first/trial_csv/exposure_control_left.csv')
	.pipe(csv())
	.on('data', data => {
	...
	.on('end', () => {
		...
		fs.createReadStream('control_left_D_first/trial_csv/test_control_left_coronal.csv')
			.pipe(csv())
			.on('data', data => {
			...
			.on('end', () => {
				...
				fs.createReadStream('control_left_D_first/trial_csv/test_control_left_labial.csv')
					.pipe(csv())
					.on('data', data => {
					...
					.on('end', () => {
						...
						fs.createReadStream('control_left_D_first/trial_csv/test_control_left_dorsal.csv')
							.pipe(csv())
							.on('data', data => {
