...
const writeResults = (word, coronal, labial, dorsal, assets) => {
	fs.writeFile('control_right_G_first/js/stims.js', buildOutput(word, coronal, labial, dorsal, assets), err => {
		if (err) {
			console.error(err)
		}
...
fs.createReadStream('control_right_G_first/trial_csv/exposure_control_right.csv')
	.pipe(csv())
	.on('data', data => {
	...
	.on('end', () => {
		console.log('done reading exposure_control_right.csv')
		fs.createReadStream('control_right_G_first/trial_csv/test_control_right_coronal.csv')
			.pipe(csv())
			.on('data', data => {
			...
			.on('end', () => {
				console.log('done reading test_control_right_coronal.csv')
				fs.createReadStream('control_right_G_first/trial_csv/test_control_right_labial.csv')
					.pipe(csv())
					.on('data', data => {
					...
					.on('end', () => {
						console.log('done reading test_control_right_labial.csv')
						fs.createReadStream('control_right_G_first/trial_csv/test_control_right_dorsal.csv')
							.pipe(csv())
							.on('data', data => {
							...
							.on('end', () => {
								console.log('done reading test_control_right_dorsal.csv')
