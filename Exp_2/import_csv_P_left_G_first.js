...
const writeResults = (word, coronal, labial, dorsal, assets) => {
	fs.writeFile('P_left_G_first/js/stims.js', buildOutput(word, coronal, labial, dorsal, assets), err => {
		if (err) {
			console.error(err)
		}
...
fs.createReadStream('P_left_G_first/trial_csv/exposure_P_left.csv')
	.pipe(csv())
	.on('data', data => {
	...
	.on('end', () => {
		console.log('done reading exposure_P_left.csv')
		fs.createReadStream('P_left_G_first/trial_csv/test_P_left_coronal.csv')
			.pipe(csv())
			.on('data', data => {
			...
			.on('end', () => {
				console.log('done reading test_P_left_coronal.csv')
				fs.createReadStream('P_left_G_first/trial_csv/test_P_left_labial.csv')
					.pipe(csv())
					.on('data', data => {
					...
					.on('end', () => {
						console.log('done reading test_P_left_labial.csv')
						fs.createReadStream('P_left_G_first/trial_csv/test_P_left_dorsal.csv')
							.pipe(csv())
							.on('data', data => {
							...
							.on('end', () => {
								console.log('done reading test_P_left_dorsal.csv')
